-- First, drop the existing functions to allow recreation with new return types
DROP FUNCTION IF EXISTS public.admin_add_pix_key(text,text,text,boolean);
DROP FUNCTION IF EXISTS public.admin_set_primary_pix_key(uuid);
DROP FUNCTION IF EXISTS public.admin_delete_pix_key(uuid);
DROP FUNCTION IF EXISTS public.admin_save_payment_settings(boolean,boolean,boolean,numeric);
